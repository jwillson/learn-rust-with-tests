# Summary

[Learn Rust with Tests](introduction.md)

# Rust Fundamentals

- [Install Rust](install-rust.md)
- [Hello, World](hello-world.md)
- [Integers](integers.md)
- [Iteration](iteration.md)
- [Arrays, vectors and slices](arrays-and-slices.md)
- [Structs, methods and traits](structs-methods-and-traits.md)
- [Ownership and borrowing](ownership-and-borrowing.md)
- [Errors and Result](errors-and-result.md)
- [Option and pattern matching](option-and-pattern-matching.md)
- [HashMaps](hashmaps.md)
- [Dependency injection](dependency-injection.md)
- [Mocking](mocking.md)
- [Concurrency](concurrency.md)
- [Select](select.md)
- [Sync](sync.md)
- [Lifetimes](lifetimes.md)
- [Traits and generics](traits-and-generics.md)
- [Iterators and closures](iterators-and-closures.md)
- [Cancellation](cancellation.md)
- [Property-based tests](property-based-tests.md)
- [Maths](maths.md)
- [Reading files](reading-files.md)
- [Templating](templating.md)

# Build an Application

- [HTTP server](http-server.md)
- [JSON, routing and embedding](json-routing-embedding.md)
- [IO and sorting](io-and-sorting.md)
- [Command line and project structure](command-line-and-project-structure.md)
- [Time](time.md)
- [WebSockets](websockets.md)

# Testing Fundamentals

- [Introduction to acceptance tests](intro-to-acceptance-tests.md)
- [Scaling acceptance tests](scaling-acceptance-tests.md)
- [Working without mocks, stubs and spies](working-without-mocks.md)
- [Refactoring checklist](refactoring-checklist.md)

# Questions and Answers

- [OS exec]()
- [Error types]()
- [Cancellation-aware Reader]()
- [Revisiting HTTP handlers]()

# Meta

- [Why unit tests and how to make them work for you]()
- [Anti-patterns]()
