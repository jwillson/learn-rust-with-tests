...
impl fmt::Display for InsufficientFundsError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "cannot withdraw, insufficient funds")
    }
}
...
mod tests {
    ...
    // ANCHOR: display_test
    #[test]
    fn describes_the_error_to_humans() {
        let error = InsufficientFundsError;

        assert_eq!(error.to_string(), "cannot withdraw, insufficient funds");
    }
    // ANCHOR_END: display_test
}
